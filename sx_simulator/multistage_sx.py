...
def solve_multistage_countercurrent(
    ...
    # NaOH 분배 (고정 NaOH 모드용)
    if NaOH_stages is None:
        NaOH_stages = list(range(1, n_stages + 1))
    n_NaOH_stages = len(NaOH_stages)
    Q_NaOH_per_stage = Q_NaOH / n_NaOH_stages if n_NaOH_stages > 0 else 0.0
    ...
    for iteration in range(max_iter):
        ...
        for s in range(n_stages):
            ...
            if t_pH is not None:
                ...
                # 고정 NaOH 모드
                q_naoh = Q_NaOH_per_stage if stage_num in NaOH_stages else 0.0
                result = solve_single_stage(
                    C_aq_in=C_aq_current, C_org_in=C_org_in,
                    pH_in=pH_current, Q_aq=Q_aq, Q_org=Q_org,
                    extractant=extractant, C_ext=C_ext,
    ...
    return {
        ...
        "total_NaOH_mol_hr": sum(NaOH_profile),
    }
