...
| 용어 | 영어 | 뜻 |
|---|---|---|
| **용매추출 (SX)** | Solvent Extraction | 두 가지 섞이지 않는 액체(물과 기름처럼)를 이용해 원하는 금속만 골라 뽑아내는 분리 기술 |
| **Mixer-Settler** | - | 용매추출 장치의 한 종류. 두 액체를 세게 섞는 부분(Mixer)과, 다시 가라앉혀 분리하는 부분(Settler)으로 구성 |
| **Stage (단)** | - | Mixer-Settler 1세트를 '1단(Stage)'이라 부름. 단수가 많을수록 추출 효율이 올라감 |
...
| **O/A 비 (Organic/Aqueous)** | - | 유기계 유량 ÷ 수계 유량. 이 비율이 크면 추출제가 상대적으로 많아져 추출률이 올라감 |
    """)
...
| 용어 | 영어 | 뜻 |
|---|---|---|
| **pH** | - | 용액의 산성/염기성 정도. 7이 중성, 낮을수록 산성, 높을수록 염기성 |
| **NaOH** | Sodium Hydroxide | 가성소다. pH를 올려(염기성으로 만들어) 금속 추출을 돕는 시약 |
...
| **Buffer Capacity** | 완충 용량 | 용액이 pH 변화에 저항하는 능력. 황산염이 많으면 pH가 쉽게 변하지 않음 |
| **황산염 (Sulfate)** | SO₄²⁻ | 배터리 리싸이클 공정에서 흔한 음이온. pH 완충 효과를 줌 |
| **Proton Balance** | 수소이온 물질수지 | 반응 전후로 H⁺(수소이온)가 얼마나 생기고 사라지는지 계산하는 것 |
...
| **mol/hr** | - | 시간당 몰 수. 화학 반응 속도를 나타내는 단위 |
    """)
...
| **Pinch Point** | - | 평형 곡선과 조작선이 거의 만나는 지점. 여기서는 단수를 아무리 늘려도 추출이 더 이상 안 됨 |
| **Sigmoid (S자 곡선)** | - | 'S'자 모양의 수학 함수. pH에 따른 추출률을 모사하는 데 사용 |
| **R²** | 결정계수 | 피팅(회귀)이 얼마나 잘 맞는지의 지표. 1.0에 가까울수록 완벽한 적합 |
    """)
...
| 기호 | 이름 | 이온 | 용도 |
|---|---|---|---|
| **Li** | 리튬 (Lithium) | Li⁺ | 2차전지 양극재·전해질 핵심 원료 |
| **Ni** | 니켈 (Nickel) | Ni²⁺ | 2차전지 양극재(고용량)의 핵심 원료 |
| **Co** | 코발트 (Cobalt) | Co²⁺ | 2차전지 양극재(안정성)의 핵심 원료 |
| **Mn** | 망간 (Manganese) | Mn²⁺ | 2차전지 양극재(원가) 및 구조 안정제 |
    """)
